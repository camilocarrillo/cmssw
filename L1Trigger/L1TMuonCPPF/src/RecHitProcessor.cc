	...
	//Loop over the look up table    
	  ...
	  if(((*cppf1).rawId == rawId) && ((*cppf1).strip == rechitstrip)){
	    ...
	    if(Geo){
	      ...
	      l1t::CPPFDigi* MainVariables1 = new l1t::CPPFDigi(rpcId, Bx , (*cppf).int_phi, (*cppf).int_theta, EMTFsector1, isValid, old_strip, clustersize, station, global_phi, global_theta);
	      l1t::CPPFDigi* MainVariables2 = new l1t::CPPFDigi(rpcId, Bx , (*cppf).int_phi, (*cppf).int_theta, EMTFsector2, isValid, old_strip, clustersize, station, global_phi, global_theta);
	      /*
